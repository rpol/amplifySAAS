...
- **Framework**: Next.js 16 (App Router), NestJS (latest), TypeScript, Tailwind CSS v4  
- **UI Components**: Shadcn UI  
- **Validation**: Zod  
- **Forms & State Management**: React Hook Form, Zustand  
- **Tables & Data Handling**: TanStack Table  
- **Tooling & DX**: Turborepo, pnpm, ESLint, Prettier, Husky  

## Monorepo layout

- `apps/web`: Next.js App Router experience
- `apps/api`: NestJS REST API (default port `3001`)
...
4. **Start the development server**
   ...
   Or, to run a single workspace directly:
   ```bash
   pnpm --filter @amplify/web dev
   pnpm --filter @amplify/api dev
   ```

Your app will be running at [http://localhost:3000](http://localhost:3000)

The API service responds at [http://localhost:3001](http://localhost:3001) with a basic health endpoint.
